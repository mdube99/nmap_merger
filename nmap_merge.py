...
def addHeader(f):
    ...
    nMap_Header += (
        "<!-- Nmap Merged with nMapMergER.py https://github.com/CBHue/nMapMergER -->"
    )
...
def addFooter(f, h):
    nMap_Footer = (
        '<runstats><finished time="1" timestr="Wed Sep  0 00:00:00 0000" elapsed="0" summary="Nmap done at Wed Sep  0 00:00:00 0000; '
        + str(h)
        + ' IP address scanned in 0.0 seconds" exit="success"/>'
    )
...
#
def main_nMapMerger(xmlSet, output_name):
    HOSTS = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--file", dest="filename", help="parse FILE", metavar="FILE"
    )
    parser.add_argument(
        "-d",
        "--dir",
        dest="directory",
        help="Parse all xml in directory",
        metavar="DIR",
    )
    parser.add_argument(
        "-q",
    ...
    parser.add_argument(
        "-o", "--output", action="store", help="Change output directory"
    )
